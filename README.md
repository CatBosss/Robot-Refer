...
### 6. lumos
* **[FastUMI Pro 示例数据集 (Hugging Face)](https://huggingface.co/datasets/FastUMIPro/example_data_fastumi_pro_raw)**:
    这是 **FastUMI** 提供的原始示例数据仓库。FastUMI 通常与**机器人通用操纵（Universal Manipulation Interface）**相关。该链接包含了一组用于测试和演示的原始机器人操作数据，开发者可以使用这些数据来测试其模仿学习算法，或了解如何格式化用于训练机器人的多模态轨迹数据。

* **[FastUMI 数据市场 (Data Market)](https://fastumi.com/data-market/)**:
    这是 FastUMI 的官方数据交易平台。它专注于提供高质量的**机器人训练数据集**，特别是针对复杂操作任务的专家轨迹数据。由于当前机器人大模型（如运动生成模型）极度依赖高质量的物理世界操作数据，该市场为开发者提供了一个获取、交换或购买特定场景（如工业组装、家庭服务等）训练数据的渠道。
